...
pub(crate) use crate::wire_emit::{LinkAuth, LinkPacket, Wire};
...
impl<S: Store> Node<S> {
    ...
    /// persisted by core, so pass it every launch (like the identity seed). Trace-hop labeling is
    ...
    fn resolve_vaccine_target(&self, token: &[u8; 32]) -> Option<BundleId> {
        for id in self.store.have().ids {
            ...
            // r13-01: the tag keys on the content id, not the wire id, so match against content_id.
    ...
    /// A fresh stream id, unique per this node. The byte layout is pinned in `wire_emit`.
    fn next_stream_id(&mut self) -> StreamId {
        self.stream_seq += 1;
        derive_stream_id(self.stream_seq, &self.identity.address())
    }
    ...
    fn prepare_delivery_custody(
        ...
        // Chunk boundaries, `seq` numbering, and `fin` placement are pinned in `wire_emit`.
        let payloads = carrier_chunk_payloads(&encoded, stream_id, chunk_bytes);
        let opts = BundleOpts {
            created_at: self.now_ms,
            flags: BundleFlags {
                request_ack: true,
        ...
        let mut custody = Vec::with_capacity(payloads.len());
    ...
    /// mesh, even multiple hops away via relays (§15 to §16). Returns the advert id so
    ...
    fn auth_payload(&self) -> Vec<u8> {
        encode_link_auth(self.identity.address())
    }
    ...
    fn offer_one_to_link(
        ...
        }
        {
            ...
            let to_send: Option<Bundle> = match self.router.should_forward(&meta, &peer) {
                ...
                // Destination is directly reachable on its own link, so deliver there,
    ...
    fn send_record(&mut self, link: LinkId, record: &Wire) {
        ...
        // Framing and fragmentation are decided in `wire_emit`; the ratchet stays here because it
        // is per-link state. `frame_record` calls `encrypt` in order and stops at the first
        // failure, so a desync abandons the remainder of the record exactly as before.
        let framed = frame_record(record, |piece| est.session.encrypt(piece).ok());
        for bytes in framed {
            self.outgoing.push((link, bytes));
        }
    }
...
mod tests {
    ...
    use crate::wire_emit::{MAX_ADVERT_LINK_BYTES, MAX_RECORD_FRAGMENTS, MAX_RECORD_PLAINTEXT};
    ...
    fn large_message_arrives_through_a_relay() {
        ...
        exchange_prekeys(&mut net, &mut nodes); // content is ratcheted: needs prekeys (§25)
    ...
    fn private_message_floods_anonymously_and_only_the_recipient_recognizes_it() {
        ...
        exchange_prekeys(&mut net, &mut nodes); // content is forward-secret: needs prekeys (§25)
    ...
    fn spoofed_private_peer_message_is_not_attributed_to_the_claimed_sender() {
        ...
        assert!(
            ...
            "the seal is well-formed and addressed to Bob, so he recognizes it"
        );
        assert!(
    ...
    fn a_forged_private_ack_cannot_mark_a_send_delivered() {
        ...
        // is moved into the node (Identity is not Clone, so reconstruct from the secret bytes).
    ...
    fn a_recognition_header_chimera_is_rejected_at_verify_so_it_cannot_occupy_a_genuine_private_id()
    {
        ...
        // garbage while keeping the SAME id, and it self-verified. At a keep-first relay that chimera won the
